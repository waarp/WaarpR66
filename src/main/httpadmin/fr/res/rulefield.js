...
function dialog_load() {
	...
	var pt = window.center({width:800,height:300});
    dialog_box.style.top = pt.y + "px";
    dialog_box.style.left = pt.x + "px";
	var dialog_content = document.getElementById('dialog_content');
	dialog_content.innerHTML = '';
	var tab=document.createElement('table');
	var row=document.createElement('tr');
	var header=document.createElement('th');
	header.textContent = "Task Type";
	row.appendChild(header);
	header=document.createElement('th');
	header.textContent = "Path argument";
	row.appendChild(header);
	header=document.createElement('th');
	header.textContent = "Delay";
	row.appendChild(header);
	header=document.createElement('th');
	header.textContent = "Comment (optional)";
	row.appendChild(header);
	tab.appendChild(row);
	for ( var j = 0; j < arrayTasks.length; j++) {
		...
		LOG, MOVE, MOVERENAME, COPY, COPYRENAME, EXEC, EXECMOVE, EXECOUTPUT, EXECJAVA, TRANSFER, VALIDFILEPATH, DELETE, LINKRENAME, RESCHEDULE, TAR, ZIP, TRANSCODE, "FTP", "RENAME", "RESTART", "UNZEROED"
		<OPTION VALUE="LOG" SELECTED>LOG</OPTION>
		<OPTION VALUE="MOVE">MOVE</OPTION>
		...
		</SELECT>
		*/
		var typeTask = document.createElement("select");
		typeTask.id = "type" + j;
		typeTask.setAttribute('name', 'type');
		var found = 0;
		for (var k = 0; k < availableTasks.length ; k++) {
			var option = document.createElement("option");
			option.setAttribute('value', availableTasks[k]);
			var text = document.createTextNode(availableTasks[k]); 
			option.appendChild(text);
			if (availableTasks[k] == arrayTasks[j][0]) {
				found = k;
				option.setAttribute("selected","selected");
			}
			typeTask.appendChild(option);
		}
		typeTask.options[found].selected = true;
		typeTask.setAttribute("onChange", "dialog_typeSelect(" + j + " )" );
		field.appendChild(typeTask);
		...
		comment.id = "comment" + j;
		comment.setAttribute('type', 'text');
		comment.setAttribute('name', 'comment');
		comment.setAttribute('size', '20');
		if (arrayTasks[j].length > 3) {
			comment.setAttribute('value', arrayTasks[j][3]);
		} else {
		...
		var oTask = document.createElement("input");
		oTask.id = "add_" + j;
		oTask.setAttribute('type', 'submit');
		oTask.setAttribute('name', 'ADD');
		oTask.setAttribute('value', 'ADD BEFORE');
		oTask.setAttribute("onClick", "dialog_add(" + j + " )" );
		field.appendChild(oTask);
		rownext.appendChild(field);
		field=document.createElement('td');
		// <input value="REMOVE" name="REMOVE" type="submit">
		oTask = document.createElement("input");
		oTask.id = "del_" + j;
		oTask.setAttribute('type', 'submit');
		oTask.setAttribute('name', 'REMOVE');
		oTask.setAttribute('value', 'REMOVE');
		oTask.setAttribute("onClick", "dialog_delete(" + j + " )" );
		field.appendChild(oTask);
		rownext.appendChild(field);
		tab.appendChild(rownext);
	}
	dialog_content.appendChild(tab);
	// <input value="ADD AFTER" name="ADD" type="submit">
	var oTask = document.createElement("input");
	oTask.id = "add_" + (arrayTasks.length);
	oTask.setAttribute('type', 'submit');
	oTask.setAttribute('name', 'ADD');
	oTask.setAttribute('value', 'ADD AFTER');
	oTask.setAttribute("onClick", "dialog_add(" + arrayTasks.length + " )" );
	dialog_content.appendChild(oTask);
	// <input value="SET" name="SET" type="submit">
	var oValid = document.createElement("input");
	oValid.id = "set";
	oValid.setAttribute('type', 'submit');
	oValid.setAttribute('name', 'SET');
	oValid.setAttribute('value', 'SET');
	oValid.onclick = function() {dialog_selectOk();};
	dialog_content.appendChild(oValid);
	// <input value="CANCEL" name="CANCEL" type="submit">
	oValid = document.createElement("input");
	oValid.id = "cancel";
	oValid.setAttribute('type', 'submit');
	oValid.setAttribute('name', 'CANCEL');
...
function dialog_typeSelect(rank) {
	var select = document.getElementById('type'+rank);
...
function dialog_open(inputName, title) {
	...
	var xml = parser.parse(value1);
	var root = xml.getRootElement();
	var tasks = root.getChildElements();
	arrayTasks = new Array(tasks.length);
	for ( var i = 0; i < tasks.length; i++) {
		var task = tasks[i];
		var type = task.getChildElement("type");
		var path = task.getChildElement("path");
		var delay = task.getChildElement("delay");
		var comment = task.getChildElement("comment");
		if (comment) {
			arrayTasks[i] = new Array(type.getText(), path.getText(), delay.getText(), comment.getText());
		} else {
			arrayTasks[i] = new Array(type.getText(), path.getText(), delay.getText(), "");
		}
	}
	dialog_load();
...
function dialog_add(rank) {
	var tempElt = new Array("LOG",'',0,'');
	if (rank == 0) {
		arrayTasks.unshift(tempElt);
	} else if (rank < arrayTasks.length) {
		var before = arrayTasks.slice(0, rank);
		before.push(tempElt);
		arrayTasks = before.concat(arrayTasks.slice(rank, arrayTasks.length));
	} else {
		arrayTasks.push(tempElt);
	}
	dialog_load();
}
// Delete a Task
function dialog_delete(rank) {
	var max = arrayTasks.length-1;
	if (rank == 0) {
		arrayTasks.shift();
	} else if (rank < max) {
		var before = arrayTasks.slice(0, rank);
		arrayTasks = before.concat(arrayTasks.slice(rank+1, arrayTasks.length));
	} else if (rank == max) {
		arrayTasks.pop();
	}
	dialog_load();
...
function dialog_selectOk() {
	...
	for ( var j = 0; j < arrayTasks.length; j++) {
		...
		var obj = document.getElementById("type" + j);
		if (obj) {
			var val = getSelectValue(obj);
			if (val && val.length > 0) {
				var sub = new marknote.Element("type");
				sub.setText(val);
				task.addContent(sub);
				obj = document.getElementById("path" + j);
				if (obj && obj.value && obj.value.length > 0) {
					sub = new marknote.Element("path");
					sub.setText(obj.value);
					task.addContent(sub);
				} else {
					sub = new marknote.Element("path");
					task.addContent(sub);
				...
				if (obj && obj.value && obj.value.length > 0) {
					sub = new marknote.Element("delay");
					sub.setText(obj.value);
					task.addContent(sub);
				} else {
					sub = new marknote.Element("delay");
				...
				if (obj && obj.value && obj.value.length > 0) {
					sub = new marknote.Element("comment");
					sub.setText(obj.value);
					task.addContent(sub);
				}
	...
	var result = root.toString("\f");
	var regexp = new RegExp("^\n\r|^\r\n|^\n|^\r|\f", "g");
	result = result.replace(regexp, "");
	dialog_input.value = result;
	dialog_close();
}
