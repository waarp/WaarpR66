...
public class LogExtendedExport implements Runnable {
    ...
    /**
     * Try first to download the exported logs, then to import (must be a host different than the
     * source one)
     * 
     * @param ruleDownload
     * @param tryimport
                    ...
                    if (tryimport && DbConstant.admin.isConnected) {
                        try {
                            ...
                            logger.warn("Cannot load the logs from " + logsFile.getAbsolutePath()
                                    + " since: " + e.getMessage());
                            throw new OpenR66ProtocolBusinessException("Cannot load the logs from "
                                    + logsFile.getAbsolutePath(),
                                    e);
                        }
                    }
                } else {
