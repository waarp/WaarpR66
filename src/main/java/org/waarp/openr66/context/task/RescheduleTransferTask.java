 ...
 * <br>
 * "-count limit" will be the limit of retry. The current value limit is taken from the
 * "transferInfo" internal code (not any more the "information of transfer")and not from the rule as
 * "{"CPTLIMIT": limit}" as JSON code. Each time this function is called, the limit value will be
 * replaced as newlimit = limit - 1 in the "transferInfo" as "{"CPTLIMIT": limit}" as JSON code.<br>
 * To ensure correctness, the value must be in the "transferInfo" internal code since this value
 * will be changed statically in the "transferInfo". If taken from the rule, it will be wrong since
 * the value will never decrease. However, a value must be setup in the rule in order to reset the
 * value when the count reach 0. <br>
 * So in the rule, "-count resetlimit" must be present, where resetlimit will be the new value set
 * when the limit reach 0, and in the "transferInfo" internal code, "{"CPTLIMIT": limit}" as JSON
 * code must be present. If one is missing, the condition is not applied. <br>
 * <br>
 ...
 * <br>
 * <B>Important note: any subsequent task will be ignored and not executed once the reschedule is
 * accepted.</B><br>
 * <br>
