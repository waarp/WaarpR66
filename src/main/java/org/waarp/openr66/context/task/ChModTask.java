...
/**
 * This task allows to change the mode of the file (as in Unix CHMOD command) according to the
 ...
 * - this final path arg should be of the form [ua][+-=][rwx] where multiple elements could be
 * specified, separated by blank character<br>
 * - u/a meaning user(Waarp system user)/all (group and other do not exist in Java), +/-/= meaning
 * add/remove/set (set means all other values are removed), r/w/x meaning Read/Write/Execute<br>
 * <br>
 ...
 * <li>a+rw</li>
 * </ul>
 * Current access mode for the application will be applied as default "user" access mode, and no
 * access mode (-rwx) will be the default for "all" access mode.<br>
 * If several access mode are set in sequence, the result will be the sum of the results, step by
 * step.<br>
