...
public abstract class AbstractTask implements Runnable {
    ...
    protected String getReplacedValue(String arg, Object[] argFormat) {
        ...
        WaarpStringUtils.replaceAll(builder, ORIGINALFULLPATH, session.getRunner()
                .getOriginalFilename());
        WaarpStringUtils.replaceAll(builder, ORIGINALFILENAME, R66File.getBasename(session.getRunner()
                .getOriginalFilename()));
        ...
        if (session.getRemoteAddress() != null) {
            WaarpStringUtils.replaceAll(builder, REMOTEHOSTADDR, session.getRemoteAddress().toString());
            WaarpStringUtils.replaceAll(builder, LOCALHOSTADDR, session.getLocalAddress().toString());
        } else {
        ...
        if (session.getLocalChannelReference() == null) {
            ...
            try {
                WaarpStringUtils.replaceAll(builder, ERRORMSG, session.getLocalChannelReference().getErrorMessage());
            } catch (NullPointerException e) {
                WaarpStringUtils.replaceAll(builder, ERRORMSG, "NoError");
            }
            try {
                WaarpStringUtils.replaceAll(builder, ERRORCODE, session.getLocalChannelReference().getCurrentCode().getCode());
            } catch (NullPointerException e) {
                WaarpStringUtils.replaceAll(builder, ERRORCODE, "-");
            }
            try {
                WaarpStringUtils.replaceAll(builder, ERRORSTRCODE, session.getLocalChannelReference().getCurrentCode().name());
            } catch (NullPointerException e) {
                WaarpStringUtils.replaceAll(builder, ERRORSTRCODE, ErrorCode.Unknown.name());
            }
        }
