 ...
 * - SIZE LT/GT/LTE/GTE/EQ number : to check the file size according to a limit (less than, greater than, less than or equal, greater than or equal, equal)<br>
 * - DFCHECK : to check if the future received file size is compatible with the space left on both working and received directories (from general configuration or rule configuration)<br>
 * <br>
...
public class FileCheckTask extends AbstractTask {
	...
	private static enum FC_COMMAND { SIZE, DFCHECK };
	private static enum FC_ARGSSIZE { LT, GT, LTE, GTE, EQ };
		...
		while (current < commands.length) {
			if (commands[current].equalsIgnoreCase(FC_COMMAND.SIZE.name())) {
				...
				if (freesize > 0 && supposelength > freesize) {
					...
					futureCompletion.setResult(new R66Result(session, false, ErrorCode.SizeNotAllowed, runner));
					futureCompletion.setFailure(new OpenR66RunnerException("File size incompatible with Working directory"));
					return;
				}
				...
				if (freesize > 0 && supposelength > freesize) {
					...
					logger.error("File length is incompatible with available space in Recv directory: " +
							supposelength + " > " + freesize);
					futureCompletion.setResult(new R66Result(session, false, ErrorCode.SizeNotAllowed, runner));
					futureCompletion.setFailure(new OpenR66RunnerException("File size incompatible with Recv directory"));
					return;
				}
				logger.debug("DEBUG: "+ supposelength + " < "+ freesize);
			} else {
