...
public class NetworkSslServerInitializer extends ChannelInitializer<SocketChannel> {
    ...
    protected void initChannel(SocketChannel ch) throws Exception {
        ...
        if (isClient) {
            ...
            // Server: possible clientAuthent, no renegotiation still
            sslHandler = getWaarpSslContextFactory().initInitializer(
                getWaarpSslContextFactory().needClientAuthentication(), false);
        }
        pipeline.addLast("ssl", sslHandler);

        pipeline.addLast("codec", new NetworkPacketCodec());
        pipeline.addLast(NetworkServerInitializer.TIMEOUT, new IdleStateHandler(0, 0,
