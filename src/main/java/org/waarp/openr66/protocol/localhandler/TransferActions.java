...
/**
 * Class to implement actions related to real transfer: request initialization, data transfer, end
 * of transfer and of request, changing filename or filesize.
 * 
...
public class TransferActions extends ServerActions {
    ...
    public void data(Channel channel, DataPacket packet)
        ...
        if (RequestPacket.isMD5Mode(session.getRunner().getMode())) {
            logger.debug("AlgoDigest: "
                    + (localChannelReference.getPartner() != null ? localChannelReference.getPartner().getDigestAlgo()
                            : "usual algo"));
     ...
     * @throws OpenR66ProtocolNotAuthenticatedException
     */
    public void endTransfer(Channel channel, EndTransferPacket packet)
                ...
                try {
                    if (!session.getRunner().isRecvThrough() && session.getFile().length() != originalSize ||
                        ...
                        R66Result result = new R66Result(new OpenR66RunnerErrorException(
                                Messages.getString("LocalServerHandler.18")), //$NON-NLS-1$
                    ...
                    for (String[] strings : rpretasks) {
                        ...
                        if (task.getType() == TaskType.CHKFILE) {
                            ...
                            if (!task.getFutureCompletion().isSuccess()) {
                                ...
                                session.setBadRunner(runner, code);
                                session.newState(ERROR);
                                logger.error("File length is not compatible with Rule or capacity {} {}", newfilename
                                        + " : " + newSize, session);
                                ...
                                try {
                                    session.setFinalizeTransfer(false, new R66Result(new OpenR66RunnerErrorException(
                                            errorPacket.getSheader()), session,
                                            true, runner.getErrorInfo(), runner));
                                } catch (OpenR66RunnerErrorException e1) {
                                    localChannelReference.invalidateRequest(new R66Result(
                                            new OpenR66RunnerErrorException(errorPacket.getSheader()), session,
                                            true, runner.getErrorInfo(), runner));
                                } catch (OpenR66ProtocolSystemException e1) {
                                    localChannelReference.invalidateRequest(new R66Result(
                                            new OpenR66RunnerErrorException(errorPacket.getSheader()), session,
