...
public class TransferActions extends ServerActions {
	...
	public void request(Channel channel, RequestPacket packet)
		...
		if (runner.isFileMoved() && runner.isSender() && runner.isInTransfer()
			...
			// and the remote host has already received the request packet
			// => Informs the receiver of the new name
			sendFilenameFilesizeChanging(packet, runner, "Will send a modification of filename due to wildcard in SelfMode: ",
	 ...
	 * @throws OpenR66RunnerErrorException
	 */
	public void requestChangeNameSize(Channel channel, String newfilename, long newSize) throws OpenR66RunnerErrorException {
		session.newState(VALID);
		DbTaskRunner runner = session.getRunner();
					...
					for (String[] strings : rpretasks) {
						...
						if (task.getType() == TaskType.CHKFILE) {
							...
							if (! task.getFutureCompletion().isSuccess()) {
								...
								session.setBadRunner(runner, code);
								session.newState(ERROR);
								logger.error("File length is not compatible with Rule or capacity {} {}", newfilename+" : "+newSize, session);
