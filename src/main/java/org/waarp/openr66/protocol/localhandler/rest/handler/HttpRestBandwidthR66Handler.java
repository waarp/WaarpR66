...
public class HttpRestBandwidthR66Handler extends HttpRestAbstractR66Handler {
		...
		try {
			if (json instanceof BandwidthJsonPacket) {//
				...
				if (setter && arguments.getMethod() != METHOD.PUT) {
					// wrong
					result.addItem(JSON_DETAIL, "Setter should be requested with a PUT method");
					setError(handler, result, ErrorCode.IncorrectCommand);
					return;
				} else if (! setter && arguments.getMethod() != METHOD.GET) {
					// wrong
					result.addItem(JSON_DETAIL, "Getter should not be requested with a GET method");
					setError(handler, result, ErrorCode.IncorrectCommand);
					return;
				}
	...
	protected ArrayNode getDetailedAllow() {
		...
		node2.put(RestArgument.JSON_COMMAND, "GetBandwidth");
		...
		node2.put(RestArgument.JSON_PATH, "/"+this.path);
		node2.put(RestArgument.JSON_COMMAND, "SetBandwidth");
