...
/**
 * This is the Object used in every communication between OpenR66 hosts. Its
 * logic is as follow:<br>
 * <ul>
 * <li>A CONTROL Part: Mandatory
 * <ul>
 * <li>1 byte: type of command</li>
 * <li>2 bytes: length of command header except those 3 first bytes (65536 bytes
 * maximum)</li>
 ...
 * <ul>
 * <li>1: 4 bytes for Command name, (length-4) bytes for arguments (generally
 * one string with whitespace), Data block could be not empty</li>
 * <li>2: 4 bytes for Command name, (length-4) bytes for arguments (generally
 * one string with whitespace)</li>
 * <li>4: 4 bytes for Command name, (length-4) bytes for arguments (could be
 * split into 8 bytes for transfer id, 8 bytes for date-time, 4 or 8 bytes for
 * values with a 8 bytes property id, ...)</li>
 * <li>8: 4 bytes for Command name, then (length-4) bytes arguments that should
 * be split into
 * <ul>
 ...
 * </ul>
 * So length = 4+8+8+x+y+z</li>
 * <li>16: 4 bytes for Command name, then arguments that should be split into
 * <ul>
 * <li>1 byte for status -first block, continuous block, last block, normal or
 * compressed, retry-,</li>
 ...
 * <li>16 bytes for MD5</li>
 * </ul>
 * So length = 4+1+16+4+4+4+4+16 = 53 bytes<br>
 * In case of retry, the position (starting from 0) in the target is equal to
 * padding length*rank.<br>
 * The Data block has a length of data length. If it is compressed, the
 * uncompressed size is final data length.</li>
 * <li>32: 4 bytes for Command name, then as for value 16, arguments that should
 * be split into
 * <ul>
 * <li>1 byte for status -first block, continuous block, last block, normal or
 * compressed, retry-,</li>
 ...
 * The Data block has a length of data length. If it is compressed, the
 * uncompressed size is final data length.</li>
 * <li>128: 4 bytes for Command name, (length-4) bytes for arguments (generally
 * one string with whitespace), Data block could be not empty</li>
 * </ul>
 * 
...
public abstract class Request {
    ...
    public ChannelBuffer toChannelBuffer() {
        finalizeHeader();
        final ChannelBuffer buffer = ChannelBuffers.wrappedBuffer(header,
