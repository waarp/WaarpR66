 ...
 * It first creates the default startup command (ConnectionCommand), then it
 * answers it is ok to accept identification (which is implied by ConnectionCommand).
 * <li>Each time a command is received:</li>
 * <ul><li>Parsing the command</li> in order to find the corresponding class that implements it.
 * <li>Checking if the command is legal now</li> such that no transfer is currently running except if
 * is a special command (like QUIT or ABORT).
 ...
 * <li>Making the final answer of the command</li> (in some cases this is a partial answer 
 * like ready to transfer) 
 * </ul>
