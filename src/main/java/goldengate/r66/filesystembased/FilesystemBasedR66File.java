 ...
 */
public class FilesystemBasedR66File extends FilesystemBasedFileImpl {
    /**
     * Internal Logger
     */
    private static final GgInternalLogger logger = GgInternalLoggerFactory
            .getLogger(FilesystemBasedR66File.class);
    /**
     * Retrieve lock
     */
     ...
     */
    public FilesystemBasedR66File(SessionInterface session,
    ...
    public long length() throws CommandAbstractException {
        ...
        // length due to block
        // XXX TODO FIXME
        final long block = (long) Math.ceil((double) length
    ...
    public void trueRetrieve() {
        ...
        try {
             ...
             * e); return; } Channel channel = ((FtpSession)
             * session).getDataConn() .getCurrentDataChannel(); DataBlock block
             * = null; try { block = readDataBlock(); } catch
             * (FileEndOfTransferException e) { // Last block (in fact, previous
             * block was the last one, // but it could be aligned with the block
             * size so not // detected) closeFile(); ((FtpSession)
             * session).getDataConn()
             * .getFtpTransferControl().setPreEndOfTransfer(); return; } if
             * (block == null) { // Last block (in fact, previous block was the
             * last one, // but it could be aligned with the block size so not
             * // detected) closeFile(); ((FtpSession) session).getDataConn()
             * .getFtpTransferControl().setPreEndOfTransfer(); return; } //
             * While not last block ChannelFuture future = null; while (block !=
             * null && !block.isEOF()) { future = Channels.write(channel,
             * block); // Test if channel is writable in order to prevent OOM if
             ...
             * session).getDataConn() .getFtpTransferControl()
             * .setPreEndOfTransfer(); return; } } else { return;// Wait for the
             * next InterestChanged } } // Last block closeFile(); if (block !=
             * null) { future = Channels.write(channel, block); } // Wait for
             * last write if (future != null) { future.awaitUninterruptibly();
             * ((FtpSession) session).getDataConn()
             * .getFtpTransferControl().setPreEndOfTransfer(); } } catch
             * (FileTransferException e) { // An error occurs! ((FtpSession)
             * session).getDataConn().getFtpTransferControl()
