...
public class TaskFactory {
    ...
    public static AbstractTask getTaskFromId(String name, String argRule,
        ...
        switch (type) {
            case TEST:
                return new TestTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case MOVE:
                return new MoveTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case MOVERENAME:
                return new MoveRenameTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case COPY:
                return new CopyTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case COPYRENAME:
                return new CopyRenameTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case EXEC:
                return new ExecTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            case EXECRENAME:
                return new ExecRenameTask(argRule, session.getRequest()
                        .getFileInformation(), session);
            default:
