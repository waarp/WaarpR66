...
public class ClientRunner extends Thread {
	...
	public LocalChannelReference initRequest()
		...
		if (restartPost) {
			...
			/*
			 * localChannelReference.setClientRunner(this); R66Result finalValue = new
			 * R66Result(localChannelReference.getSession(), true, ErrorCode.CompleteOk,
			 * taskRunner); localChannelReference.sessionNewState(R66FiniteDualStates.ENDREQUESTS);
			 * taskRunner.quickFinalizeOnPost(localChannelReference, finalValue); EndRequestPacket
			 * validPacket = new EndRequestPacket(ErrorCode.CompleteOk.ordinal()); if (!
			 * taskRunner.isSender()) {
			 * localChannelReference.sessionNewState(R66FiniteDualStates.ENDREQUESTR);
			 * validPacket.validate(); } try {
			 * ChannelUtils.writeAbstractLocalPacket(localChannelReference,
			 * validPacket).awaitUninterruptibly(); } catch (OpenR66ProtocolPacketException e) { }
			 * if (!localChannelReference.getFutureRequest().awaitUninterruptibly(
			 * Configuration.configuration.TIMEOUTCON)) { // valid it however
			 * taskRunner.setAllDone(); try { taskRunner.saveStatus(); } catch
			 * (OpenR66RunnerErrorException e) { // ignore }
			 * localChannelReference.validateRequest(localChannelReference
			 * .getFutureEndTransfer().getResult()); } logger.debug("Wait for request to {}",host);
			 * host = null; return localChannelReference;
			 */
		}
