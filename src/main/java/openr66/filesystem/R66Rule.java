...
public class R66Rule {
     ...
     */
    public static final ConcurrentHashMap<String, R66Rule> hashMap =
        new ConcurrentHashMap<String, R66Rule>();
    /**
     * Global Id
     */
    ...
    /**
     * The associated Error Tasks (with passed code 0=pretask, 1=file, 2=posttask)
     */
    ...
    public R66Rule(String idrule, String ids, String recvpath, String sendpath,
                    String archivepath, String workpath,
                    String pretasks, String posttasks, String errortasks) {
            this.assign(idrule,ids,recvpath,sendpath,archivepath,workpath,pretasks,posttasks,
                            errortasks);
    }
    /**
     * Create a Rule from args
     ...
     */
    public R66Rule(String idrule, String []idsArray, String recvpath, String sendpath,
                    String archivepath, String workpath,
                    String [][]pretasksArray, String [][]posttasksArray, String [][]errortasksArray) {
            this.assign(idrule,idsArray,recvpath,sendpath,archivepath,workpath,pretasksArray,
                            posttasksArray,errortasksArray);
    }
    /**
     * Get Ids from String. If it is not ok, then it sets the default values
    ...
    private boolean getIdsRule(String idsref) {
            ...
            try {
                    ...
                    logger.warn("Unable to read the ids for Rule: "+
                                    idsref, e);
                    logger.info("No ids so setting to the default!");
                    this.ids = null;
                    this.idsArray = null;
            ...
            this.idsArray = new String[listNode.size()];
            int i = 0;
            for (Node nodeid : listNode) {
                    this.idsArray[i] = nodeid.getText();
                    i++;
            }
            listNode.clear();
            listNode = null;
    ...
    private String [][] getTasksRule(String tasks) {
            ...
            try {
                    ...
                    logger.warn("Unable to read the tasks for Rule: "+
                                    tasks, e);
                    logger.info("No tasks so setting to the default!");
                    reader.close();
                    return null;
    ...
    private static String setIdsRule(String []idsArray) {
            ...
            if (idsArray != null) {
                    ...
                    for (int i = 0; i < idsArray.length; i++) {
                            ids+=XMLHOSTID+idsArray[i]+XMLENDHOSTID;
                    }
                    ids += XMLENDHOSTIDS;
            }
            return ids;
    ...
    private static String setTasksRule(String [][]tasksArray) {
            ...
            if (tasksArray != null) {
                    ...
                    for (int i = 0; i < tasksArray.length; i++) {
                            tasks+=XMLTASK+"<"+TASK_RANK+">"+i+"</"+TASK_RANK+"><"+TASK_TYPE+">"+
                            tasksArray[i][0]+"</"+TASK_TYPE+"><"+TASK_PATH+">"+
     ...
     * @param errortasks
     */
    private void assign(String idrule, String idsref, String recvpath, String sendpath, String archivepath, String workpath,
                    String pretasks, String posttasks, String errortasks) {
            this.idRule = idrule;
            this.ids = idsref;
            ...
            this.getIdsRule(idsref);
            this.preTasksArray = this.getTasksRule(pretasks);
            this.postTasksArray = this.getTasksRule(posttasks);
            this.errorTasksArray = this.getTasksRule(errortasks);
    }
    /**
     * Assign an Rule from args
     * @param idrule
    ...
    private void assign(String idrule, String []idsArrayRef, String recvpath,
                    ...
                    String [][]pretasksArray, String [][]posttasksArray, String [][]errortasksArray) {
            this.idRule = idrule;
            this.idsArray = idsArrayRef;
            this.recvPath = recvpath;
            this.sendPath = sendpath;
            this.archivePath = archivepath;
            this.workPath = workpath;
            this.preTasksArray = pretasksArray;
            this.postTasksArray = posttasksArray;
            ids = setIdsRule(idsArrayRef);
            this.preTasks = setTasksRule(pretasksArray);
            this.postTasks = setTasksRule(posttasksArray);
            this.errorTasks = setTasksRule(errortasksArray);
    }
    /**
     ...
     */
    public static R66Rule getHash(String idrule) throws OpenR66ProtocolNoDataException {
            R66Rule rule = getInternalHash(idrule);
            if (rule == null) {
                    /*R66DbPreparedStatement p_get_id = setPGet(admin, idrule);
                    if (p_get_id == null)
                            return null;
                    ResultSet resultSet = p_get_id.executeQuery();
                    if (R66DbPreparedStatement.getNext(resultSet)) {
                            rule = new R66Rule();
                            if (rule.get(resultSet)) {
                                    setInternalHash(rule);
                            } else {
                                    rule = null;
                            }
                    }
                    resultSet = null;
                    p_get_id.realClose();*/
                throw new OpenR66ProtocolNoDataException("Cannot find rule: "+idrule);
            }
            return rule;
    }
    /**
    ...
    public static boolean del(String idrule) {
            ...
            /*R66DbPreparedStatement p_del_id = null;
            p_del_id = setPDel(admin, idrule);
            if (p_del_id == null)
                    return false;
            if (p_del_id.executeUpdate() == 1) {*/
                    retour = true;
    ...
    public boolean set() {
            boolean retour = false;
            /*R66DbPreparedStatement p_get_r66 = setPGet(admin, this.idRule);
            if (p_get_r66 == null)
                    return false;
            ResultSet resultSet =
                    p_get_r66.executeQuery();
            int nb = 0;
            if (R66DbPreparedStatement.getNext(resultSet)) {
                    resultSet = null;
                    p_get_r66.realClose();
                    resultSet = null;
                    R66DbPreparedStatement p_upd_r66 =
                            setPUpdate(admin, idRule, ids, recvPath, sendPath, archivePath,
                                            workPath, preTasks, postTasks, errorTasks);
                    if (p_upd_r66 == null)
                            return false;
                    nb = p_upd_r66.executeUpdate();
                    p_upd_r66.realClose();
                    if (nb > 0) {*/
                            retour = true;
                    /*}
    ...
    public boolean insert() {
            boolean retour = false;
            /*int nb = 0;
            R66DbPreparedStatement preparedStatement = setPInsert(admin, idRule, ids, recvPath, sendPath, archivePath, workPath,
    ...
    public String setRecvPath(String filename) throws OpenR66ProtocolSystemException {
            ...
            return FileUtils.consolidatePath(Configuration.configuration.inPath,filename);
    }
    /**
    ...
    public String setSendPath(String filename) throws OpenR66ProtocolSystemException {
            ...
            return FileUtils.consolidatePath(Configuration.configuration.outPath, filename);
    }
    /**
    ...
    public String setArchivePath(String filename) throws OpenR66ProtocolSystemException {
            ...
            return FileUtils.consolidatePath(Configuration.configuration.archivePath, filename);
    }
    /**
    ...
    public String setWorkingPath(String filename) throws OpenR66ProtocolSystemException {
            if (workPath != null) {
                    return workPath+R66Dir.SEPARATOR+filename+Configuration.EXT_R66;
            }
            return FileUtils.consolidatePath(Configuration.configuration.workingPath, filename);
    }
    /**
    ...
    public boolean checkHostAllow(String hostId) {
            ...
            for (int i = 0; i < this.idsArray.length; i++) {
                    if (this.idsArray[i].equalsIgnoreCase(hostId)) {
                            return true;
                    }
            }
            return false;
    ...
    public String toString() {
            return "Rule Name:"+idRule+" IDS:"+ids+" RECV:"+recvPath+" SEND:"+sendPath+" ARCHIVE:"+archivePath+
                    " WORK:"+workPath+" PRET:"+preTasks+" POST:"+postTasks+" ERROR:"+errorTasks;
    }
}
