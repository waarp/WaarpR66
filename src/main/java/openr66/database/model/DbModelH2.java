...
import openr66.database.data.R66DbConfiguration;
...
public class DbModelH2 extends AbstractDbModel {
    ...
    public void createTables() {
        ...
        // Configuration
        String action = createTableH2+R66DbConfiguration.table+"(";
        R66DbConfiguration.Columns []columns = R66DbConfiguration.Columns.values();
        for (int i = 0; i < columns.length-1; i++) {
            action += columns[i].name()+
                DBType.getType(R66DbConfiguration.dbTypes[i])+
                notNull+", ";
        }
        action += columns[columns.length-1].name()+
            DBType.getType(R66DbConfiguration.dbTypes[columns.length-1])+
            primaryKey+")";
