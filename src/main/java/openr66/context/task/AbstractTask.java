...
public abstract class AbstractTask implements Runnable {
	...
	protected String getReplacedValue(String arg, Object[] argFormat) {
		...
		if (session.getRemoteAddress() != null) {
			GgStringUtils
					.replaceAll(builder, REMOTEHOSTADDR, session.getRemoteAddress().toString());
		...
		if (session.getLocalChannelReference() == null) {
			...
			try {
				GgStringUtils.replaceAll(builder, ERRORMSG, session.getLocalChannelReference()
						.getErrorMessage());
			} catch (NullPointerException e) {
				GgStringUtils.replaceAll(builder, ERRORMSG, "NoError");
			}
			try {
				GgStringUtils.replaceAll(builder, ERRORCODE, session.getLocalChannelReference()
						.getCurrentCode().getCode());
			} catch (NullPointerException e) {
				GgStringUtils.replaceAll(builder, ERRORCODE, "-");
			}
			try {
				GgStringUtils.replaceAll(builder, ERRORSTRCODE, session.getLocalChannelReference()
						.getCurrentCode().name());
			} catch (NullPointerException e) {
				GgStringUtils.replaceAll(builder, ERRORSTRCODE, ErrorCode.Unknown.name());
			}
