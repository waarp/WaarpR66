...
public class AuthenticationFileBasedConfiguration {
		...
		while (iterator.hasNext()) {
			...
			if (value == null || (value.isEmpty())) {
				...
				if (value == null || (value.isEmpty())) {
					...
					if (skey.length() > 0) {
						try {
							byteKeys = Configuration.configuration.cryptoKey
									.decryptHexInBytes(skey);
						} catch (Exception e) {
	 ...
	 */
	public static void writeXML(String filename) throws OpenR66ProtocolSystemException,
			GoldenGateDatabaseNoConnectionError, GoldenGateDatabaseSqlError {
